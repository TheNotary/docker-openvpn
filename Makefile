...
configure:
	...
	docker run --volumes-from ${OVPN_DATA} --rm ${USER}/${DOCKER_IMAGE_NAME} ovpn_genconfig -u udp://VPN.SERVERNAME.COM && \
	docker run --volumes-from ${OVPN_DATA} --rm -it ${USER}/${DOCKER_IMAGE_NAME} ovpn_initpki && \
  docker run --volumes-from ${OVPN_DATA} --rm -it ${USER}/${DOCKER_IMAGE_NAME} easyrsa build-client-full CLIENTNAME nopass && \
  docker run --volumes-from ${OVPN_DATA} --rm ${USER}/${DOCKER_IMAGE_NAME} ovpn_getclient CLIENTNAME > CLIENTNAME.ovpn
...
run:
	docker run \
  --volumes-from ${OVPN_DATA} -d -p 1194:1194/udp --cap-add=NET_ADMIN ${USER}/${DOCKER_IMAGE_NAME}
